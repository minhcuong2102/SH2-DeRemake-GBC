...
void remove_projectile(void) NONBANKED {
    if (projectile->flags & UPDATE_VAR) {
        ...
        *(script_memory + projectile_delta_x) = SUBPX_TO_PX(projectile->pos.x);
        *(script_memory + projectile_delta_y) = SUBPX_TO_PX(projectile->pos.y);
    }
...
void handle_anchor(void) BANKED {
    ...
    y_offset += projectile->delta_pos.y;
    // switch (projectile->dir) {
    switch (actors[projectile_actor].dir) {
            case DIR_RIGHT:
            ...
            //     projectile->pos.x =
            //         projectile->origin_x +
            //         ((projectile->phase + projectile->x) << 7) +
            //         x_offset;
            ...
            //     projectile->pos.y =
            //         projectile->origin_y -
            //         ((projectile->phase + projectile->y) << 7) -
            //         y_offset;
            //     break;
...
void handle_bounce(void) BANKED {
    ...
    WORD tile_x = SUBPX_TO_TILE(projectile->pos.x) + 1;
    WORD tile_y = SUBPX_TO_TILE(projectile->pos.y);
    ...
    while (projectile) {
        if (!check_stop()) {
            ...
            if (IS_FRAME_EVEN) {
                ...
                (projectile->flags & IGNORE_PLAYER) ? &PLAYER : NULL);
        ...
        UBYTE screen_y = SUBPX_TO_PX(projectile->pos.y) - draw_scroll_y;
...
void projectile_launch(UBYTE index, upoint16_t *pos, UBYTE angle) BANKED {
    ...
    if (projectile) {
        ...
        // projectile->def.anim_noloop = (flags & PROJECTILE_ANIM_NOLOOP);
        // projectile->def.strong = (flags & PROJECTILE_STRONG);
        ...
        switch (projectile->type) {
            ...
            //replace with abs, if i can find it..
                ...
                // projectile->origin_x = projectile->pos.x;
                // projectile->origin_y = projectile->pos.y;
                // projectile->delta_pos.x = (projectile->delta_pos.x < 0) ? -projectile->delta_pos.x : projectile->delta_pos.x;
                // projectile->delta_pos.y = (projectile->delta_pos.y < 0) ? -projectile->delta_pos.y : projectile->delta_pos.y;
                break;
        }
